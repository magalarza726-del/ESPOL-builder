...
const hud = {
  distance: $('#distance'), altitude: $('#altitude'), speed: $('#speed'),
  bearing: $('#bearing'), objective: $('#objective'), viewMode: $('#viewMode'),
  coords: $('#coords'), pace: $('#pace')
};
const perf = {
  fps: $('#perfFps'), ms: $('#perfMs'), draws: $('#perfDraws'), tris: $('#perfTris'),
  veg: $('#perfVeg'), chunks: $('#perfChunks'), quality: $('#perfQuality')
};
const toast = $('#toast');
let toastTimer;
let gameWorld = null;
...
const state = {
  cameraMode: 'follow', terrainEnabled: true, treesEnabled: true,
  buildingsEnabled: true, imageryEnabled: false, season: .25,
  lastFrame: performance.now(), visited: new Set(), draggingLook: false,
  lastPointerX: 0, lastPointerY: 0, pitchLook: 0, forestData: null,
  moveForward: 0, moveRight: 0, currentSpeed: 0, turnRate: 0,
  lastHudUpdate: 0, lastLandmarkCheck: 0, lastMapMarkerUpdate: 0,
  lastPerfUpdate: 0
};
const keys = new Set();
...
function haversine(a, b) {
  const R = 6371000, p1 = rad(a.lat), p2 = rad(b.lat);
...
function terrainElevation(lng, lat) {
  ...
  try { return map.queryTerrainElevation([lng, lat], { exaggerated: false }); }
  catch { return null; }
}
function insideCampus(lng, lat) {
  return lng >= CAMPUS.bounds.west && lng <= CAMPUS.bounds.east &&
    lat >= CAMPUS.bounds.south && lat <= CAMPUS.bounds.north;
}
...
function moveBy(forward, right, meters) {
  ...
  const resolved = gameWorld
    ? gameWorld.resolvePosition(before.lng, before.lat, target.lng, target.lat, CAMPUS.playerRadiusM || .38)
    : { ...target, collided: false };
  player.lng = resolved.lng;
  player.lat = resolved.lat;
  player.totalM += haversine(before, player);
  if (resolved.collided) state.currentSpeed = Math.min(state.currentSpeed, BASE_SPEED_MPS * .75);
}
function moveStable(forward, right, distanceM) {
  const maxStep = Math.max(.35, CAMPUS.movementSubstepM || .75);
...
function updateHUD() {
  ...
  if (hud.pace) hud.pace.textContent = player.speedMps > .15
    ? (player.speedMps > BASE_SPEED_MPS * 1.08 ? 'Sprint ×2.5' : 'Movimiento base ×3')
    : 'Quieto';
}
function updateProfiler() {
  if (!gameWorld || state.cameraMode === 'map') {
    perf.fps.textContent = 'MAP'; perf.ms.textContent = '—'; perf.draws.textContent = '—';
    perf.tris.textContent = '—'; perf.veg.textContent = '—'; perf.chunks.textContent = '—';
    perf.quality.textContent = 'GIS'; return;
  }
  const s = gameWorld.getStats();
  perf.fps.textContent = Math.round(s.fps);
  perf.ms.textContent = `${s.frameMs.toFixed(1)} ms`;
  perf.draws.textContent = s.drawCalls;
  perf.tris.textContent = s.triangles > 999 ? `${(s.triangles / 1000).toFixed(1)}k` : s.triangles;
  perf.veg.textContent = s.vegetation;
  perf.chunks.textContent = s.chunks;
  perf.quality.textContent = `${Math.round(s.quality * 100)}%`;
}
function setMapInteractions(enabled) {
  for (const name of ['dragPan', 'scrollZoom', 'boxZoom', 'doubleClickZoom', 'touchZoomRotate', 'dragRotate', 'touchPitch']) {
    const h = map[name]; if (!h) continue;
    try { enabled ? h.enable() : h.disable(); } catch { }
  }
  try { map.keyboard.disable(); } catch { }
}
function focusCampus({ duration = 500 } = {}) {
  const padding = Math.max(32, Math.min(78, Math.round(Math.min(innerWidth, innerHeight) * .065)));
...
function syncMapPresentation() {
  ...
  if (map.getLayer('espol-imagery')) {
    ...
    map.setPaintProperty('espol-imagery', 'raster-opacity', show ? .72 : 0);
  }
}
function setCameraMode(mode, { notifyUser = true } = {}) {
  ...
  if (mode === 'map') {
    map.resize(); focusCampus();
    if (notifyUser) notify('Mapa GIS: zoom/pan libres dentro de ESPOL');
  } else {
    map.stop(); gameWorld?.resetCamera?.();
    if (notifyUser) notify(mode === 'follow' ? 'Mundo 3D · colisiones activas' : 'Primera persona · colisiones activas');
  }
...
function renderProgress() {
  const targets = LANDMARKS.slice(0, 3), wrap = $('#progressDots');
  wrap.innerHTML = '';
  for (const l of targets) {
    const i = document.createElement('i');
    if (state.visited.has(l.id)) i.classList.add('done');
    wrap.appendChild(i);
  }
...
function gameLoop(now) {
  const dt = Math.min(.045, Math.max(0, (now - state.lastFrame) / 1000));
  state.lastFrame = now;
  let rawForward = 0, rawRight = 0, rawRotate = 0;
  if (keys.has('KeyW')) rawForward += 1;
  if (keys.has('KeyS')) rawForward -= 1;
  if (keys.has('KeyA')) rawRight -= 1;
  if (keys.has('KeyD')) rawRight += 1;
  ...
  state.moveForward = expApproach(state.moveForward, rawForward, CAMPUS.inputResponsiveness || 19, dt);
  state.moveRight = expApproach(state.moveRight, rawRight, CAMPUS.inputResponsiveness || 19, dt);
  ...
  const accel = targetSpeed > state.currentSpeed ? (CAMPUS.accelerationMps2 || 34) : (CAMPUS.brakingMps2 || 46);
  ...
  if (state.currentSpeed > 0 && smoothLen > .001) moveStable(state.moveForward / smoothLen, state.moveRight / smoothLen, state.currentSpeed * dt);
  ...
  if (now - state.lastHudUpdate > (CAMPUS.hudIntervalMs || 100)) { state.lastHudUpdate = now; updateHUD(); }
  if (now - state.lastLandmarkCheck > (CAMPUS.landmarkIntervalMs || 280)) { state.lastLandmarkCheck = now; checkLandmarks(); }
  if (now - state.lastPerfUpdate > 450) { state.lastPerfUpdate = now; updateProfiler(); }
...
function addCampusBounds() {
  ...
  map.addLayer({ id:'campus-fill', type:'fill', source:'campus-bounds', paint:{ 'fill-color':'#7c9b72', 'fill-opacity':.015 } });
  map.addLayer({ id:'campus-outline', type:'line', source:'campus-bounds', paint:{ 'line-color':'#587a61', 'line-width':1.2, 'line-opacity':.48 } });
...
function addTerrain() {
  ...
  map.addLayer({id:'terrain-hillshade',type:'hillshade',source:'terrain-dem',paint:{'hillshade-exaggeration':.08,'hillshade-shadow-color':'#71806a','hillshade-highlight-color':'#eef0e7','hillshade-accent-color':'#9da58f'}});
}
function addImagery() {
  ...
  map.addLayer({id:'espol-imagery',type:'raster',source:'espol-world-imagery',minzoom:14,layout:{visibility:'none'},paint:{'raster-opacity':.72,'raster-saturation':-.25,'raster-contrast':-.02,'raster-fade-duration':0}},labelId);
}
function addBuildings() {
  ...
  map.addLayer({id:'espol-buildings-3d',source:'ofm','source-layer':'building',type:'fill-extrusion',minzoom:14.2,filter:['!=',['get','hide_3d'],true],paint:{'fill-extrusion-color':'#c8cbc5','fill-extrusion-height':height,'fill-extrusion-base':base,'fill-extrusion-opacity':.92,'fill-extrusion-vertical-gradient':false}},labelId);
}
function addForest() {
  state.forestData=buildProceduralForest();
  map.addSource('procedural-forest',{type:'geojson',data:state.forestData});
  map.addLayer({id:'forest-mass',type:'heatmap',source:'procedural-forest',maxzoom:17.3,paint:{
    'heatmap-weight':['case',['==',['get','habit'],'tree'],['interpolate',['linear'],['coalesce',['get','clusterSize'],1],1,.45,22,1.85],.22],
    'heatmap-intensity':['interpolate',['linear'],['zoom'],14,.46,16.5,1.18],
    'heatmap-radius':['interpolate',['linear'],['zoom'],14,13,16.8,32],
    'heatmap-opacity':['interpolate',['linear'],['zoom'],14,.28,16.4,.48,17.3,0],
    'heatmap-color':['interpolate',['linear'],['heatmap-density'],0,'rgba(42,72,43,0)',.25,'rgba(65,100,56,.24)',.55,'rgba(50,91,49,.48)',.82,'rgba(36,77,43,.67)',1,'rgba(28,67,39,.78)']
  }});
  map.addLayer({id:'forest-individuals',type:'circle',source:'procedural-forest',minzoom:16.8,filter:['==',['get','habit'],'tree'],paint:{'circle-radius':['interpolate',['linear'],['zoom'],16.8,.8,18,1.7,20,2.8],'circle-color':['case',['==',['get','zone'],'campus'],'#73896a',['==',['get','deciduous'],1],'#6f784e','#49664b'],'circle-opacity':['interpolate',['linear'],['zoom'],16.8,.14,19,.42],'circle-stroke-width':0}});
}
...
function waitForMapIdle(timeout=650){return new Promise(resolve=>{let done=false;const finish=()=>{if(done)return;done=true;resolve();};map.once('idle',finish);setTimeout(finish,timeout);});}
function structureAccumulator(){return {buildings:[],roads:[],bSeen:new Set(),rSeen:new Set()};}
function collectLoadedStructures(acc){
  try{
    for(const f of map.querySourceFeatures('ofm',{sourceLayer:'building'})){
      let minLng=Infinity,maxLng=-Infinity,minLat=Infinity,maxLat=-Infinity;
      eachCoord(f.geometry?.coordinates,c=>{minLng=Math.min(minLng,c[0]);maxLng=Math.max(maxLng,c[0]);minLat=Math.min(minLat,c[1]);maxLat=Math.max(maxLat,c[1]);});
      if(!Number.isFinite(minLng))continue;
      const lng=(minLng+maxLng)/2,lat=(minLat+maxLat)/2;if(!insideCampus(lng,lat))continue;
      const width=(maxLng-minLng)*111320*Math.cos(lat*DEG),depth=(maxLat-minLat)*110574;
      if(width<1.5||depth<1.5||width>300||depth>300)continue;
      const p=f.properties||{},height=num(p.render_height)||num(p.height)||(num(p.levels)?num(p.levels)*3.15:null)||7.4;
      const key=`${lng.toFixed(5)}:${lat.toFixed(5)}:${Math.round(width)}:${Math.round(depth)}`;
      if(acc.bSeen.has(key))continue;acc.bSeen.add(key);acc.buildings.push({lng,lat,width,depth,height});
    }
  }catch(err){console.warn('building extraction',err);}
  try{
    const widthFor=p=>{const cls=(p.class||p.subclass||'').toLowerCase();if(cls.includes('motorway'))return 9;if(cls.includes('trunk'))return 8;if(cls.includes('primary'))return 7;if(cls.includes('secondary'))return 6;if(cls.includes('tertiary'))return 5;if(cls.includes('residential'))return 4;if(cls.includes('service'))return 3;if(cls.includes('path')||cls.includes('foot'))return 1.3;if(cls.includes('track'))return 2.2;return 3.2;};
    for(const f of map.querySourceFeatures('ofm',{sourceLayer:'transportation'})){
      const lines=[];collectLines(f.geometry?.coordinates,lines);
      for(const line of lines)for(let i=1;i<line.length;i++){
        const a=line[i-1],b=line[i],lng=(a[0]+b[0])/2,lat=(a[1]+b[1])/2;if(!insideCampus(lng,lat))continue;
        const key=`${a[0].toFixed(5)}:${a[1].toFixed(5)}:${b[0].toFixed(5)}:${b[1].toFixed(5)}`;
        if(acc.rSeen.has(key))continue;acc.rSeen.add(key);acc.roads.push({a,b,width:widthFor(f.properties||{})});if(acc.roads.length>=7500)return;
      }
    }
  }catch(err){console.warn('road extraction',err);}
}
function fallbackBuildings(){
  const defs=[['aud-fiec',48,30,9],['fiec-stop',58,32,10],['fiec-11b',46,26,9],['fiec-11f',38,23,8],['aud-fimcp',46,30,10],['fimcp-24c',48,26,9],['postgrado',42,28,10],['terminal',74,28,6],['coliseo-nuevo',82,56,13],['biblioteca',58,42,13],['rectorado',48,36,11]];
  return defs.map(([id,width,depth,height])=>{const l=LANDMARKS.find(x=>x.id===id);return l?{lng:l.lng,lat:l.lat,width,depth,height,fallback:true}:null;}).filter(Boolean);
}
function mergeFallbackBuildings(acc){for(const b of fallbackBuildings()){let near=false;for(const e of acc.buildings){if(haversine({lng:b.lng,lat:b.lat},{lng:e.lng,lat:e.lat})<24){near=true;break;}}if(!near)acc.buildings.push(b);}}
async function scanCampusStructures(){
  const acc=structureAccumulator();
  ...
  for(const lat of ys)for(const lng of xs){
  ...
  mergeFallbackBuildings(acc);focusCampus({duration:0});
  return {buildings:acc.buildings.slice(0,3000),roads:acc.roads.slice(0,7500)};
}

function bindLookControls(){
  const canvas=$('#game3d');
  canvas.addEventListener('mousedown',e=>{if(state.cameraMode==='map')return;state.draggingLook=true;state.lastPointerX=e.clientX;state.lastPointerY=e.clientY;e.preventDefault();});
  window.addEventListener('mousemove',e=>{if(!state.draggingLook||state.cameraMode==='map')return;const dx=e.clientX-state.lastPointerX,dy=e.clientY-state.lastPointerY;state.lastPointerX=e.clientX;state.lastPointerY=e.clientY;player.bearing=normBearing(player.bearing+dx*.22);state.pitchLook=clamp(state.pitchLook-dy*.12,-55,55);});
...
function bindUI(){
  ...
  $('#gameMode').onchange=e=>{document.body.classList.remove('mode-horror','mode-rpg','mode-shooter');if(e.target.value!=='explore')document.body.classList.add(`mode-${e.target.value}`);notify(`Preset ${e.target.options[e.target.selectedIndex].text} activado`);};
...
map.on('load',async()=>{
  try{
    loadingStep(8,'Preparando Mapa ESPOL…');lockMapToCampus();addTerrain();addImagery();addBuildings();addForest();addCampusBounds();addLandmarks();focusCampus({duration:0});
    loadingStep(18,'Generando bosque ecológico de alta densidad…');
    const worldPromise=createGameWorld({canvas:$('#game3d'),forestData:state.forestData,terrainUrl:MAP_SOURCES.terrain,onProgress:(p,total)=>loadingStep(20+p*40,`Precalculando relieve ESPOL · ${Math.round(p*100)}% · ${total} tiles`)});
    const structures=await scanCampusStructures();
    gameWorld=await worldPromise;
    loadingStep(84,`Construyendo campus · ${structures.buildings.length} edificios · ${structures.roads.length} tramos`);
    gameWorld.setStructures(structures);gameWorld.setSeason(state.season);gameWorld.setTreesEnabled(state.treesEnabled);gameWorld.setBuildingsEnabled(state.buildingsEnabled);gameWorld.setTerrainEnabled(state.terrainEnabled);
    loadingStep(94,'Activando colisiones de edificios y árboles…');bindUI();$('#imageryToggle').checked=false;setCameraMode('follow',{notifyUser:false});updateHUD();updateProfiler();loadingStep(100,'ESPOL Builder listo');
    setTimeout(()=>{loading.classList.add('hide');state.lastFrame=performance.now();requestAnimationFrame(gameLoop);notify(`v0.7 · bosque denso · ${structures.buildings.length} edificios · colisiones activas`);},350);
  }catch(err){console.error(err);loadingText.textContent='Falló la inicialización del mundo. Revisa la consola.';setTimeout(()=>loading.classList.add('hide'),1800);}
});
