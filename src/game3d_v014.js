...
import { installFIMCPPhotoReconstruction } from './fimcp-detail-v015.js';
...
function eachCoord(coords, fn) {
  ...
  if (coords.length >= 2 && typeof coords[0] === 'number' && typeof coords[1] === 'number') { fn(coords); return; }
  for (const child of coords) eachCoord(child, fn);
}
function capturedBuildingRecords() {
  ...
  for (const feature of features) {
    ...
    out.push({ ...feature, lng: (minLng + maxLng) * .5, lat: (minLat + maxLat) * .5, height, levels });
  }
  return out;
}
...
export async function createGameWorld(options) {
  ...
  world.setStructures = structures => {
    ...
    // FIMCP v0.15: the photographic reconstruction only decorates the same GIS
    // footprints already used by Map/Three.js. It never creates an alternative
    // campus coordinate system or a second collision model.
    installFIMCPPhotoReconstruction(world, capturedBuildingRecords(), structures);
