...
function clusterOffset(p,i){
  ...
  const id=(+p.id||1)*37+i*101,angle=hash01(id)*Math.PI*2,r=Math.sqrt(hash01(id+17))*Math.max(2,+p.clusterSpreadM||8);
...
class TerrariumHeightCache{
  ...
  async decodeTile(x,y){
    ...
    try{
      ...
      const blob=await res.blob();let image;if('createImageBitmap' in window)image=await createImageBitmap(blob);else image=await new Promise((resolve,reject)=>{const img=new Image();img.crossOrigin='anonymous';img.onload=()=>resolve(img);img.onerror=reject;img.src=URL.createObjectURL(blob)});
      const canvas=typeof OffscreenCanvas!=='undefined'?new OffscreenCanvas(256,256):Object.assign(document.createElement('canvas'),{width:256,height:256}),ctx=canvas.getContext('2d',{willReadFrequently:true});ctx.drawImage(image,0,0,256,256);const data=ctx.getImageData(0,0,256,256).data;
...
function vegetationMeshes(capacity){
  const trunk=makeInstanced(new THREE.CylinderGeometry(1,1,1,5),capacity.trees,flatMat(0x674d39)),white=flatMat(0xffffff);
  ...
  const cross=makeCrossGeometry();return {
    trunk,canopies,
    ...
    herb:makeInstanced(cross,capacity.herbs,flatMat(0xffffff,{double:true})),vine:makeInstanced(cross,capacity.vines,flatMat(0xffffff,{double:true})),epiphyte:makeInstanced(new THREE.OctahedronGeometry(.5,0),capacity.epiphytes,white.clone())
  };
}
...
function terrainColor(lng,lat,elev){const academic=lng>-79.9707&&lng<-79.9575&&lat>-2.1530&&lat<-2.1395;if(academic)return new THREE.Color(0x777b61);if(elev>210)return new THREE.Color(0x536445);if(elev>155)return new THREE.Color(0x5b6b49);return new THREE.Color(0x63704b)}
...
class GameWorld{
  constructor(canvas,heightCache,forestData){
    ...
    this.renderer=new THREE.WebGLRenderer({canvas,antialias:false,alpha:false,powerPreference:'high-performance',precision:'mediump'});this.renderer.setClearColor(0x89999b,1);this.renderer.setPixelRatio(1);this.renderer.outputColorSpace=THREE.SRGBColorSpace;
    ...
    this.buildingColliders=[];this.buildingGrid=new Map();this.buildingCellM=80;
    ...
    this.resize();window.addEventListener('resize',()=>this.resize());this.buildTerrain();this.buildLake();
  }
  ...
  buildTerrain(){
    const west=(CAMPUS.bounds.west-CAMPUS.spawn.lng)*METERS_LNG,east=(CAMPUS.bounds.east-CAMPUS.spawn.lng)*METERS_LNG,north=-(CAMPUS.bounds.north-CAMPUS.spawn.lat)*METERS_LAT,south=-(CAMPUS.bounds.south-CAMPUS.spawn.lat)*METERS_LAT,cols=4,rows=3,seg=30,mat=flatMat(0xffffff,{vertexColors:true});this.terrainChunks=[];
  ...
  buildLake(){const l=LANDMARKS.find(x=>x.id==='lake');if(!l)return;const p=lngLatToLocal(l.lng,l.lat),y=this.getElevation(l.lng,l.lat)-this.originElev+.22,g=new THREE.CircleGeometry(1,28),m=flatMat(0x557f86,{double:true}),mesh=new THREE.Mesh(g,m);mesh.rotation.x=-Math.PI/2;mesh.scale.set(115,72,1);mesh.position.set(p.x,y,p.z);this.scene.add(mesh)}
  setStructures({buildings=[],roads=[]}={}){
    ...
    if(buildings.length){
      const mesh=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),flatMat(0xbfc3bc),buildings.length),q=new THREE.Quaternion();mesh.instanceMatrix.setUsage(THREE.StaticDrawUsage);
      buildings.forEach((b,i)=>{const p=lngLatToLocal(b.lng,b.lat),h=clamp(b.height||7.5,2.5,55),w=Math.max(2,b.width||8),d=Math.max(2,b.depth||8),y=this.getElevation(b.lng,b.lat)-this.originElev+h/2;this.tmpP.set(p.x,y,p.z);this.tmpS.set(w,h,d);this.tmpM.compose(this.tmpP,q,this.tmpS);mesh.setMatrixAt(i,this.tmpM);const col={x:p.x,z:p.z,hx:w*.5,hz:d*.5};this.buildingColliders.push(col);gridInsert(this.buildingGrid,this.buildingCellM,col)});
      mesh.instanceMatrix.needsUpdate=true;mesh.frustumCulled=true;this.buildingGroup.add(mesh);
    }
    if(roads.length){
      const mesh=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),flatMat(0x686c69),roads.length),axis=new THREE.Vector3(0,1,0);mesh.instanceMatrix.setUsage(THREE.StaticDrawUsage);
      roads.forEach((r,i)=>{const a=lngLatToLocal(r.a[0],r.a[1]),b=lngLatToLocal(r.b[0],r.b[1]),dx=b.x-a.x,dz=b.z-a.z,len=Math.max(.5,Math.hypot(dx,dz)),lng=(r.a[0]+r.b[0])/2,lat=(r.a[1]+r.b[1])/2,y=this.getElevation(lng,lat)-this.originElev+.10,angle=Math.atan2(dx,dz);this.tmpP.set((a.x+b.x)/2,y,(a.z+b.z)/2);this.tmpS.set(r.width||3,.10,len);this.tmpQ.setFromAxisAngle(axis,angle);this.tmpM.compose(this.tmpP,this.tmpQ,this.tmpS);mesh.setMatrixAt(i,this.tmpM)});
      mesh.instanceMatrix.needsUpdate=true;mesh.frustumCulled=true;this.roadGroup.add(mesh);
    }
    this.buildingGroup.visible=this.buildingsEnabled;this.stats.colliders=this.buildingColliders.length;
  }
  ...
  refreshVegetation(x,z,now){
    ...
    for(const [d2,f] of rows){const p=f.properties,h=p.habit||'tree';if(h==='tree'){
      if(d2<=tree2)treeRows.push([d2,f]);
      else if(massN<budget.mass){const y=this.getElevation(f.geometry.coordinates[0],f.geometry.coordinates[1])-this.originElev,hgt=Math.max(4,+p.height||7),can=Math.max(2,+p.canopyM||3),cl=Math.max(5,+p.clusterSize||10),scale=can*(1.7+Math.min(2.2,Math.sqrt(cl)*.32));this.instance(this.veg.canopyMass,massN++,p._x,y+hgt*.68,p._z,scale,scale*.48,scale,treeColor(p,this.season))}
    }else if(d2<=under2&&under[h]&&under[h].length<budget[h])under[h].push([d2,f])}
    ...
    for(const [,f] of treeRows){
      const p=f.properties,[lng,lat]=f.geometry.coordinates,baseY=this.getElevation(lng,lat)-this.originElev,copies=Math.max(1,Math.min(+p.clusterSize||1,22));
      for(let j=0;j<copies&&ti<budget.tree;j++){
        const off=clusterOffset(p,j),jitter=.78+hash01((+p.id||1)*71+j*13)*.46,h=Math.max(2.5,(+p.height||7)*jitter),can=Math.max(1.0,(+p.canopyM||2.4)*(.82+hash01(j+(+p.id||1)*9)*.36)),tr=Math.max(.035,(+p.trunkRadiusM||.08)*(.72+hash01(j*5+(+p.id||1))* .55)),tx=p._x+off.x,tz=p._z+off.z;
        this.instance(this.veg.trunk,ti,tx,baseY+h*.34,tz,tr,h*.68,tr,null);const form=this.veg.canopies[p.form]?p.form:'round',mesh=this.veg.canopies[form],idx=ci[form]++,loss=this.season<.5&&+p.deciduous===1?.66+this.season*.38:1;let sx=can,sy=can*.85,sz=can;if(form==='umbrella'){sx*=1.2;sy*=.5;sz*=1.2}else if(form==='open'){sx*=.88;sy*=.72;sz*=.88}else if(form==='ceibo'){sx*=1.08;sy*=.72;sz*=1.08}this.instance(mesh,idx,tx,baseY+h*.74,tz,sx*loss,sy*loss,sz*loss,treeColor(p,this.season));ti++;
      }
    ...
    const fill=(rows,mesh,habit)=>{let n=0,cap=Math.min(mesh.userData.capacity,budget[habit]);for(const [,f] of rows){const p=f.properties,vis=+p.seasonalDry===1?1-.55*this.season:clamp((+p.dryPersistence||.45)+(1-(+p.dryPersistence||.45))*this.season,0,1);if((+p.rand||.5)>vis)continue;const lng=f.geometry.coordinates[0],lat=f.geometry.coordinates[1],y=this.getElevation(lng,lat)-this.originElev,s=Math.max(.22,+p.scale||.6),copies=Math.min(+p.density||1,habit==='herb'?5:habit==='shrub'?3:2);for(let j=0;j<copies&&n<cap;j++){const a=(+p.rand||.5)*6.283+j*2.1,sp=habit==='herb'?1.35:.72,xx=p._x+Math.cos(a)*sp*j*.45,zz=p._z+Math.sin(a)*sp*j*.45;let sx=s,sy=s,sz=s;if(habit==='herb'){if(p.form==='grass'){sx=.22*s;sy=1.45*s;sz=.22*s}else if(p.form==='fern'){sx=1.15*s;sy=.72*s;sz=1.15*s}else if(p.form==='cactus'){sx=.35*s;sy=1.25*s;sz=.35*s}else{sx=.65*s;sy=.88*s;sz=.65*s}}else if(habit==='vine'){sx=.22*s;sy=2.1*s;sz=.22*s}else if(habit==='epiphyte'){sx=.4*s;sy=.28*s;sz=.4*s}else{sx=1.05*s;sy=.82*s;sz=1.05*s}this.instance(mesh,n++,xx,y+(habit==='epiphyte'?.8:0),zz,sx,sy,sz,understoryColor(p,this.season))}}mesh.count=n;mesh.instanceMatrix.needsUpdate=true;if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;return n};
  ...
  blockedBuilding(x,z,r){if(!this.buildingsEnabled)return false;for(const b of this.buildingCandidates(x,z))if(Math.abs(x-b.x)<b.hx+r&&Math.abs(z-b.z)<b.hz+r)return true;return false}
  blockedTree(x,z,r){
    if(!this.treesEnabled)return false;const rows=queryGrid(this.index,x,z,16);for(const [,f] of rows){const p=f.properties;if((p.habit||'tree')!=='tree')continue;const copies=Math.max(1,Math.min(+p.clusterSize||1,22));for(let j=0;j<copies;j++){const o=clusterOffset(p,j),dx=x-(p._x+o.x),dz=z-(p._z+o.z),rr=r+Math.max(.11,+p.trunkRadiusM||.08);if(dx*dx+dz*dz<rr*rr)return true}}return false;
  }
  ...
  resolvePosition(fromLng,fromLat,toLng,toLat,radius=.38){
    ...
    if(!this.blocked(b.x,a.z,radius)){const ll=localToLngLat(b.x,a.z);return {...ll,collided:true}}
    if(!this.blocked(a.x,b.z,radius)){const ll=localToLngLat(a.x,b.z);return {...ll,collided:true}}
    return {lng:fromLng,lat:fromLat,collided:true};
  }

  adaptPerformance(dt){this.perfTime+=dt;this.perfFrames++;if(this.perfTime<2.2)return;const fps=this.perfFrames/this.perfTime;this.stats.fps=fps;this.stats.frameMs=1000/Math.max(1,fps);let next=this.dynamicFactor;if(fps<42)next-=.12;else if(fps<50)next-=.06;else if(fps>58&&this.dynamicFactor<this.hardwareFactor)next+=.05;next=clamp(next,.42,this.hardwareFactor);if(Math.abs(next-this.dynamicFactor)>.02){this.dynamicFactor=next;this.stats.quality=next;this.forceRefresh=true;this.resize()}this.perfTime=0;this.perfFrames=0}
  render(player,state,dt,now){
    this.setSeason(state.season);if(this.treesEnabled!==state.treesEnabled)this.setTreesEnabled(state.treesEnabled);this.buildingGroup.visible=state.buildingsEnabled;this.terrainGroup.visible=state.terrainEnabled;const p=lngLatToLocal(player.lng,player.lat),ground=this.getElevation(player.lng,player.lat)-this.originElev,y=ground,forward=new THREE.Vector3(Math.sin(player.bearing*DEG),0,-Math.cos(player.bearing*DEG));
